<h1 align="center">Hi 👋, I'm Ratman</h1>
<h3 align="center">I'm just learning how to do some stuff</h3>

# Some Info

- 🔭 I’m currently working on **HOI4 mod about my fictional country**

- 🌱 I’m currently learning **HOI4 modding, C++ and some more as side projects**

- 🐣 I have almost **none coding skills** for **now** (maybe i'd be able to make something in **scratch**)

- ⚡ Fun fact **grass is green**

# My Goals
<h3>Current Goals</h3>

- 💻 Learn **C++**

- 🎮 Finish **HOI4** mod

- 🛠️ Learn using **microcontrollers** and programming them in either **C++** or **micropython**

- 🐧 Learn using Linux
# 
<h3>Future Goals</h3>

- 🌐 Learn **Typescript** (maybe bit of web development with that)

- 🖥️ Learn **C**

- 💡 Learn about **electronics**

- 🧩 Learn **X86-64 Assembly** (and maybe **6502**)

- ⚡ Make **simple CPU** using **transistors** or **logic gates**

- 🤖 Make something that **works** and is **functional** with **6502 microprocessor** (or similar microcontroller)

<p align="left">
</p>
